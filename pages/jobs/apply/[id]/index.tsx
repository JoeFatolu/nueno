...
export default function Jobs() {
  ...
  const { isLoading, data } = useQuery(["jobs", jobUid], getApplicationDetails);
  async function getApplicationDetails() {
    const response = await axios.get("/api/jobs/" + jobUid);
    const responseData: Job = response.data;
    return responseData;
  }
  if (!jobUid) {
  ...
  return (
    <Shell>
      <header>
        <div className="py-6 mx-auto max-w-7xl sm:px-6 md:px-0">
          <h1 className="text-3xl font-bold text-gray-900">Apply For Job</h1>
          <h2 className="text-3xl font-bold text-gray-900">Role: {data.title}</h2>
          <p className="text-3xl font-bold text-gray-900">Description: {data.description}</p>
          <span className="sm:ml-3"></span>
        </div>
      </header>
      <main>
        <form className="mb-10 space-y-8 divide-y divide-gray-200">
          <div className="space-y-8 divide-y divide-gray-200 sm:space-y-5">
            ...
            <div className="pt-8 space-y-6 divide-y divide-gray-200 sm:pt-10 sm:space-y-5">
              <div>
                ...
                <p className="max-w-2xl mt-1 text-sm text-gray-500">
                  We&rsquo;ll always let you know about important changes, but you pick what else you want to
                  hear about.
                </p>
              </div>
